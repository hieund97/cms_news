@if($posts->currentPage() == 1)
    @foreach($posts as $post)
        ...
        @endphp
        <a class="text-decoration-none" href="{{route('fe.post',['slug'=>$post->slug,'id'=>$post->id])}}">
            <div class="row mt-2 pb-3 border-bottom">
                ...
                <div class="col-md-9 mt-md-0 mt-lg-0 col-6">
                    <div class="d-flex justify-content-between flex-column h-100">
                        <h3 class="h6 font-weight-bold  text-dark mobile-post_widget-list">
                            {{$post->title}}
                        </h3>
                        <div class="d-flex align-items-center justify-content-between post-ago">
                            @foreach($post->categories as $category)
                                <span
                                    class="d-block text-decoration-none font-weight-bold text-gray rounded mr-3"
                                >{{ $category->title }}
                                </span>
                            @endforeach
                            <div class="">
        ...
        @if($loop->index == 4)
            @include('front_end.posts.elements.post-video')
        @endif
    @endforeach
@else
    @foreach($posts as $post)
