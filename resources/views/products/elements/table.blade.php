        ...
        <tbody>
            @foreach($products as $product)
                <tr>
                    <td>{{ $product->id }}</td>
                    <td>
                        @if(!empty($product->productMedias[0]->url))
                            <img style="max-width: 150px; word-break: normal;" src="{{ get_image_url($product->productMedias[0]->url, 'thumb') }}" alt="{{ $product->name }}" class="image-column">
                        @endif
                    </td>
                    <td style="white-space: normal;">
                        <b>{{ $product->name }}</b>
                        <a href="javascript:" onclick="showHistories('{{ route('products.logs', ['product' => $product->id]) }}')" class="ml-1">
                        ...
                        @if(!empty($used))
                            ...
                            <br />
                        @endif
                        <a target="_blank" href="{{ Route('fe.product',["slug"=>$product->slug,"id"=>$product->id]) }}"> Link xem chi tiết </a>
                    </td>
                                ...
                                <div class="form-group clearfix margin-bottom-10">
                                    @if(empty($typeDelete))
                                        ...
                                        @can('products.destroy')
                                            <a href="javascript:" class="btn btn-danger btn-sm" onclick="deleteResource('{{ route('products.destroy', ['product' => $product->id]) }}', '{{ route('products.index') }}')">
                                                {{ __('Delete') }}
                                            </a>
                                        @endcan
                                        @else
                                        <a href="javascript:" class="btn btn-warning btn-sm restore_btn_dtb" data-url="{{route('products.delete.restore',['id'=>$product->id])}}">
                                            {{ __('Restore') }}
                                        </a>
                                    @endif
                                </div>
...
@push('footer')
    <div class="modal fade" id="historiesModal" tabindex="-1" role="dialog" aria-labelledby="historiesModalTitle" aria-hidden="true">
