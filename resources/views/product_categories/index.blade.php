...
@section('content')
    <div class="row">
        ...
        @if ($showForm)
            <div class="col-md">
                <form method="post" class="card card-primary card-outline card-outline-tabs" action="{{ !empty($currentCategory) ? route('product_categories.update', ['product_category' => $currentCategory->id]) : route('product_categories.store') }}" enctype="multipart/form-data">
                    @csrf
