...
class PostController extends Controller
{
    ...
    public function index()
    {
        ...
        $featurePosts = Post::where('status', array_search('publish', Post::STATUS))
            ...
            ->take(6);
        /* Set meta */
        $metaTitle = 'Tin tức đồ chơi ô tô cập nhật 24h - Chungauto.vn';
        $metaDescription = 'Thông tin đồ chơi ô tô, phụ tùng ô tô, thiết bị mới nhất được cập nhật hằng giờ. Tin về sản phẩm mới mắt, đánh giá sản phẩm, mẹo hay sử dụng...';
        ...
        meta()->set('title', $metaTitle)
            ...
            ->set('keywords', 'Tin tức, tin ô tô, tin đồ chơi ô tô, đánh giá sản phẩm')
    ...
    {
        $post = Post::with(['categories.posts', 'postTags.posts'])->where('status', array_search('publish', Post::STATUS))->findOrFail($id);
        ...
        $tags = $post->postTags;
        $randomPosts = Post::where('status', array_search('publish', Post::STATUS))
            ->where('id', '!=', $id)
            ->inRandomOrder()
            ->get()
            ->take(6);
    ...
    {
        $postCategories = Category::where('id', '!=', $id)->get(['id', 'title', 'slug'])->take(12);
